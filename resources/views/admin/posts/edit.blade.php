<x-layouts.admin title="Edit Post">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold mt-2" style="color: var(--color-heading);">Edit Post</h1>
    </div>
    ...
    <form id="post-form" method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="slug" value="{{ old('slug', $post->slug) }}" class="form-input">
        </div>

        <div>
            <label class="block text-sm font-medium mb-1" style="color: var(--color-heading);">Excerpt</label>
            <textarea name="excerpt" rows="2" class="form-input" style="resize: none;">{{ old('excerpt', $post->excerpt) }}</textarea>
        </div>
        ...
        <div>
            ...
            @if ($post->featured_image)
                <div class="mb-3">
                    <img src="{{ Storage::url($post->featured_image) }}" alt="Current image" class="h-32 rounded object-cover" style="border-radius: var(--radius-btn);">
                    <label class="flex items-center gap-2 mt-2 text-xs" style="color: var(--color-text-muted);">
                ...
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="publish_status" value="published"
                               @checked(old('publish_status', $post->isPublished() ? 'published' : ($post->published_at ? 'scheduled' : 'draft')) === 'published')
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="publish_status" value="scheduled"
                               @checked(old('publish_status', $post->isPublished() ? 'published' : ($post->published_at ? 'scheduled' : 'draft')) === 'scheduled')
                               class="accent-gray-900">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="publish_status" value="draft"
                               @checked(old('publish_status', $post->isPublished() ? 'published' : ($post->published_at ? 'scheduled' : 'draft')) === 'draft')
                               class="accent-gray-900">
            ...
            <div x-show="open" x-cloak class="px-5 py-5 space-y-5 border-t" style="border-color: var(--color-border);">
                ...
                <div>
                    ...
                    <textarea name="meta_description" rows="3" maxlength="320"
                              ...
                              class="form-input" style="resize: none;">{{ old('meta_description', $post->meta_description) }}</textarea>
                ...
                <div>
                    ...
                    @if($post->og_image)
                        <div class="mb-3">
                            <img src="{{ Storage::url($post->og_image) }}" alt="OG image" class="h-24 rounded object-cover" style="border-radius: var(--radius-btn);">
                            <label class="flex items-center gap-2 mt-2 text-xs" style="color: var(--color-text-muted);">
                                <input type="checkbox" name="remove_og_image" value="1"> Remove current OG image
                            </label>
                        </div>
                    @endif
