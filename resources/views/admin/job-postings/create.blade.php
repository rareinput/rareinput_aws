<x-layouts.admin title="New Job Posting">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold mt-2" style="color: var(--color-heading);">New Job Posting</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.job-postings.store') }}" class="space-y-6" style="max-width: 760px;">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="location" value="{{ old('location') }}" required
                       placeholder="e.g. Remote, Mumbai, IN"
                       class="form-input">
            </div>
        </div>
            ...
            <div>
                ...
                <select name="experience" required class="form-input">
                    ...
                    @foreach ($experiences as $exp)
                        <option value="{{ $exp->value }}" @selected(old('experience') === $exp->value)>
                            {{ $exp->value }}
                        </option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <textarea name="description" rows="6" required class="form-input" style="resize: vertical;">{{ old('description') }}</textarea>
        </div>
